package svcomp.nanoxml_eqchk;/* NonValidator.java                                               NanoXML/Java
 *
 * $Revision: 1.5 $
...
/**
 * NonValidator processes the DTD and handles entity references. It does not do any validation
 * itself.
 *
...
public class NonValidator implements IXMLValidator {
  ...
  public void parseDTD(String publicID, IXMLReader reader) throws IOException {
    ...
    while ((peLevel > 0) || (!reader.atEOFOfCurrentStream())) {
      ...
      if (ch == '<') {
        ...
        throw new XMLParseException(reader.getLineNr(), "Invalid char: " + ch);
      }
   ...
   * @param reader the reader to read the DTD from.
   */
  protected void processElement(IXMLReader reader) throws IOException {
   ...
   * @param reader the reader to read the DTD from.
   */
  protected void processAttList(IXMLReader reader) throws IOException {
   ...
   * @param reader the reader to read the DTD from.
   */
  protected void processEntity(IXMLReader reader) throws IOException {
  ...
  protected char readChar(IXMLReader reader, boolean resolvePE) throws IOException {
    ...
    if (resolvePE && (ch == '%')) {
      ...
      if (value == null) {
        throw new XMLParseException(reader.getLineNr(), "Invalid entity: `&" + key + ";'");
        ...
        try {
          ...
          throw new XMLParseException(reader.getLineNr(), "Could not open " + id[1]);
        }
      }
    } else {
      return ch;
   ...
   * @param reader the reader to read the DTD from.
   */
  protected void skipComment(IXMLReader reader) throws IOException {
    if (this.readChar(reader, true) != '-') {
      this.skipTag(reader);
      return;
    }
   ...
   * @param reader the reader to read the DTD from.
   */
  protected void skipTag(IXMLReader reader) throws IOException {
    int level = 1;
   ...
   * @param reader the reader to read the DTD from.
   */
  protected String scanIdentifier(IXMLReader reader) throws IOException {
   ...
   * @param reader the reader to read the DTD from.
   */
  protected String scanString(IXMLReader reader) throws IOException {
    StringBuffer result = new StringBuffer();
    char delim = this.readChar(reader, true);

    if ((delim != '\'') && (delim != '"')) {
      throw new XMLParseException(reader.getLineNr(), "String need to be delimited");
    }
   ...
   */
  protected void skipWhitespace(IXMLReader reader, boolean allowPercent) throws IOException {
    char ch;
